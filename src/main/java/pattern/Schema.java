...
public class Schema extends LinkedHashMap<String, DataField> implements
		Serializable {
	/** Field LOG */
	private static final Logger LOG = LoggerFactory.getLogger(Schema.class);

	/**
	 * Field label_field - metadata of the label to be produced by the
	 * classifier
	 */
	 ...
	 */
	public void parseDictionary(PMML pmml, NodeList node_list)
			throws PatternException {
		for (int i = 0; i < node_list.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (!containsKey(name)) {
					DataField df = DataFieldFactory.getDataField(
							pmml.getReader(), node, name, op_type, data_type);
					put(name, df);
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (containsKey(name)) {
					...
					String message = String
							.format("unknown DataField referenced in PMML [ %s ]",
									name);
					LOG.error(message);
	...
	public Fields getInputFields() {
		return new Fields(getParamNames());
	}

	/**
	 * Returns the expected name for each field in the Tuple, to be used as
	 * Janino parameters.
	 * 
	 * @return String[]
	 */
	...
	/**
	 * Returns the expected class for each field in the Tuple, to be used as
	 * Janino parameters.
	 * 
	 * @return Class[]
	 */
	...
	/**
	 * Convert values for the fields in the Tuple, in a form that Janino
	 * expects.
	 * 
	 * @param values
	 * @param param_values
	 * @throws PatternException
	 */
	public void setParamValues(TridentTuple values, Object[] param_values)
			throws PatternException {
		Iterator<DataField> iter = values().iterator();
	 ...
	 * @throws PatternException
	 */
	public Map<String, Object> getParamMap(TridentTuple values)
			throws PatternException {
