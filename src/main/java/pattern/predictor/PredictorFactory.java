...
public class PredictorFactory {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(PredictorFactory.class);

	/**
	 * Create the appropriate Predictor object based on parsing the predictor
	 * terms (IV) of a regression model.
	 * 
	 * @param schema
	 *            model schema
	 * @param node
	 *            predictor node in the XML
	 * @return Predictor
	 * @throws PatternException
	 */
	public static Predictor getPredictor(Schema schema, Element node)
			throws PatternException {
		Predictor pred = null;
		String name = node.getAttribute("name");

		if ("NumericPredictor".equals(node.getNodeName())) {
			// <NumericPredictor name="petal_width" exponent="1"
			// coefficient="-0.302988044352268"/>
