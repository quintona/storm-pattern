...
public class CategoricalDataField extends DataField {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(CategoricalDataField.class);
	...
	public void parse(XPathReader reader, Node node) {
		...
		NodeList node_list = (NodeList) reader.read(node, node_expr,
	...
	public String getEval(XPathReader reader, Element node)
		...
		String expr = "./Array[1]";
		NodeList node_list = (NodeList) reader.read(node, expr,
				XPathConstants.NODESET);
	...
	public Object getValue(TridentTuple values, int i) {
		...
		LOG.debug(String.format("%s @ %d | %s", field_value, bit_index,
				categories));
		return bit_index;
	}
}
